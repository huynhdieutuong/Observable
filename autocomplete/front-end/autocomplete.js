...
const resultEl = document.getElementById('js-results')
let queryString

window.inputEl$ = Observable.fromEvent(inputEl, 'keydown')
  .debounceTime(500)
  .filter((e) => e.target.value.length > 2)
  .map((e) => {
    queryString = e.target.value
    const url = `http://localhost:8888/data?search=${queryString}`
    return new Observable.fetch(url)
  })
  .forEach((fetchOb$) => {
    resultEl.innerHTML = renderLoading()
    fetchOb$.forEach((data) => showData(data))
  })
...
function showData(data) {
  if (data.length) {
    let result = ''
    data.forEach((x) => {
      result += `<div class="list-group-item">${smartHightLightText(
        x.label
      )}</div>`
    })
    ...
    resultEl.innerHTML = renderIconEmpty()
  }
}
...
function hightLightText(text) {
  const arrString = text.toLowerCase().split(queryString.toLowerCase())
  const highlightString = `<span class="hight-light">${queryString}</span>`
  const newText = arrString.join(highlightString)

  const newArr = newText.split(' ')
  const finalText = newArr
    .map((string) => string.charAt(0).toUpperCase() + string.slice(1))
    .join(' ')

  const finalArr = finalText.split(' <span Class="hight-light">')
  return finalArr
    .map((string) => string.charAt(0).toUpperCase() + string.slice(1))
    .join(' <span class="hight-light">')
}
